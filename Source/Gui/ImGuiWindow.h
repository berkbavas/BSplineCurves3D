...
namespace BSplineRenderer
{
    ...
    class ImGuiWindow : public QObject
    {
        Q_OBJECT
      public:
        explicit ImGuiWindow(QObject* pParent);

        void Draw();

        void SetRendererManager(RendererManager* pManager);
        void SetCurveContainer(CurveContainer* pContainer);

      signals:
        void CurveAdded(SplinePtr pSpline);
