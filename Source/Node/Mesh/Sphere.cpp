...
BSplineRenderer::Sphere::Sphere(float Radius, int StackCount, int SectorCount)
{
...
void BSplineRenderer::Sphere::CreateGeometry(float Radius, int StackCount, int SectorCount)
{
    Vertex TempVertex;
    ...
    for (int i = 0; i < StackCount; ++i)
    {
        for (int j = 0; j < SectorCount; ++j)
        {
            ...
            if (i != 0)
            {
                mFaces.push_back(Sphere::TriangleFace{ First, Second, First + 1 });
            }

            if (i != (StackCount - 1))
            {
                mFaces.push_back(Sphere::TriangleFace{ First + 1, Second, Second + 1 });
            }
        }
    }
